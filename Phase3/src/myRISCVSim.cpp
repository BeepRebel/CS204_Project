...
// Function for taking input parameters
tuple<string, bool, bool, bool, bool, pair<bool, int>> take_input() {
    std::string prog_mc_file;
    bool pipelining_enabled = false;
    bool forwarding_enabled = false;
    bool print_registers_each_cycle = false;
    bool print_pipeline_registers = false;
    std::pair<bool, int> print_specific_pipeline_registers = {false, 10};
    
     // Default return values
    return {"../test/test.mc", true , true , false , true , {false , 10}};
    std::cout << "Enter MC file name: ";
    std::cin >> prog_mc_file;
    
    int choice;
    std::cout << "Enable pipelining? (1/0): ";
    std::cin >> choice;
    pipelining_enabled = (choice == 1);
    
    std::cout << "Enable forwarding? (1/0): ";
    std::cin >> choice;
    forwarding_enabled = (choice == 1);
    
    std::cout << "Print registers each cycle? (1/0): ";
    std::cin >> choice;
    print_registers_each_cycle = (choice == 1);
    
    std::cout << "Print pipeline registers? (1/0): ";
    std::cin >> choice;
    print_pipeline_registers = (choice == 1);
    
    std::cout << "Print specific pipeline registers? (1/0): ";
    std::cin >> choice;
    print_specific_pipeline_registers.first = (choice == 1);
    
    if (print_specific_pipeline_registers.first) {
        std::cout << "Enter instruction number: ";
        std::cin >> print_specific_pipeline_registers.second;
    }

    return make_tuple(prog_mc_file, pipelining_enabled, forwarding_enabled, 
            print_registers_each_cycle, print_pipeline_registers, 
            print_specific_pipeline_registers);
}
...
int main() {    
    ...
    auto result = take_input();
    string prog_mc_file = std::get<0>(result);
    bool pipelining_enabled = std::get<1>(result);
    bool forwarding_enabled = std::get<2>(result);
    bool print_registers_each_cycle = std::get<3>(result);
    bool print_pipeline_registers = std::get<4>(result);
    pair<bool,int> print_specific_pipeline_registers = std::get<5>(result);

    // Initialize processor and supporting objects
    ...
    if (!pipelining_enabled) {
        ...
        while (!prog_end) {
            ...
            if (print_specific_pipeline_registers.first) {
                for (auto& inst : pipeline_instructions) {
                    if (inst->PC / 4 == print_specific_pipeline_registers.second) {
                        if (!print_registers_each_cycle) {
                            std::cout << "CLOCK CYCLE: " << clock_cycles << std::endl;
                        }
            ...
            else if (print_pipeline_registers) {
                if (!print_registers_each_cycle) {
                    std::cout << "CLOCK CYCLE: " << clock_cycles << std::endl;
                }
                std::cout << "Pipeline Registers:-" << std::endl;
    ...
    std::vector<std::string> stats = {
        ...
        "CPI: ",
        "Number of data-transfer(load and store) instructions executed: ",
    ...
    s[2] = s[0] / (s[1] > 0 ? s[1] : 1);  // Avoid division by zero
    ...
    if (prog_end) {
        ...
        for (size_t i = 0; i < pc_tmp.size(); i++) {
            ...
            for (int j = 0; j < 5; j++) {
                if (forwarding_enabled && pipelining_enabled) {
